...
def get_pred_df(dataloader,model,ids_to_labels,device,add_true_class=True):
    ...
    probs = model.predict(dataloader,device=device)
    ...
    for preds in probs:
        ...
        final_preds.append(pred_class.tolist())
        final_scores.append(pred_scr.tolist())

    for j in range(len(dataloader.dataset.samples)):
...
class Infer(TorchModule):
    ...
    def fit(self,container,params):
        ...
        container.add_item('submission_df',df,'df_csv',mode='write')
