...
class PrepareData(Module):
    ...
    def prepare(self,container,params):
        ...
        container.train_ner_df['ent'] = container.train_ner_df['ent'].apply(lambda x: eval(x))
        container.test_ner_df['ent'] = container.test_ner_df['ent'].apply(lambda x: eval(x))
        ...
        train_set = SlideWindowDataset(container.train_ner_df,tokenizer,params['maxlen'],True,labels_to_ids,params['tokenizer_args'],params['window_size'])
        train_loader = DataLoader(train_set, batch_size=params['train_bs'],collate_fn=SlideWindowDataset.collate_fn)
        ...
        val_set = SlideWindowDataset(container.test_ner_df,tokenizer,params['maxlen'],True,labels_to_ids,params['tokenizer_args'],params['window_size'])
        val_loader = DataLoader(val_set, batch_size=params['val_bs'],collate_fn=SlideWindowDataset.collate_fn)
