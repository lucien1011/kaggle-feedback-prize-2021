...
class Preprocess(Module):
    ...
    def fit(self,container,params):
        ...
        train_df = df[df["kfold"] != params['fold']].reset_index(drop=True)#.sample(1000)
        valid_df = df[df["kfold"] == params['fold']].reset_index(drop=True)#.sample(1000)
        ...
        train_set = FeedbackDataset(train_samples, params['max_len'], tokenizer, target_id_map)
        train_loader = DataLoader(train_set, batch_size=params['train_bs'])
        
        val_set = FeedbackDataset(valid_samples, params['max_len'], tokenizer, target_id_map)
        ...
        container.add_item('train_samples',train_samples,'torch_dataset',mode='read')
        ...
        container.add_item('valid_samples',valid_samples,'torch_dataset',mode='read')
